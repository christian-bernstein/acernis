...
public class JsonDocument implements IDocument<JsonDocument> {
    ...
    public JsonDocument append(Reader reader) {
        return append(new JsonParser().parse(reader).getAsJsonObject());
    }
    ...
    public JsonDocument read(Reader reader) {
        try (BufferedReader bufferedReader = new BufferedReader(reader)) {
            return this.append(new JsonParser().parse(bufferedReader).getAsJsonObject());
        } catch (Exception ex) {
            ex.getStackTrace();
        }
        return this;
    ...
    public IReadable read(byte[] bytes) {
        this.append(new JsonParser().parse(new String(bytes, StandardCharsets.UTF_8)).getAsJsonObject());
        return this;
    }

    public JsonDocument read(String input) {
        try {
            this.append(new JsonParser().parse(new BufferedReader(new StringReader(input))).getAsJsonObject());
        } catch (Exception exception) {
            exception.printStackTrace();
        }
        return this;
